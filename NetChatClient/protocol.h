...
// 工具函数：将删除好友响应打包为载荷 (success, message)
inline QByteArray packDeleteFriendResponse(bool success, const QString &message)
{
    QByteArray payload;
    QDataStream out(&payload, QIODevice::WriteOnly);
    out.setVersion(QDataStream::Qt_6_0);
...
inline void unpackDeleteFriendResponse(const QByteArray &payload, bool &success, QString &message)
{
    QDataStream in(payload);
    in.setVersion(QDataStream::Qt_6_0);
    in >> success >> message;
